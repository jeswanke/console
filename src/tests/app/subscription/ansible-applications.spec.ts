  ...
  () => {
    ...
    test(
      ...
      async ({
        ...
        const { applicationName, namespace } = options;
        const secretName =
          options.perBlock?.[0]?.automation?.existingAnsibleSecret ?? 'ansible-pre-post-1560';
    ...
    test(
      ...
      async ({ page, oc }) => {
        ...
        await createAnsibleTowerCredentialViaCredentialsTab(credentialsPage, page, credentialSpec);
