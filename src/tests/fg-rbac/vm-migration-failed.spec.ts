...
test.describe('FG-RBAC - Failed Migration & Recovery', { tag: ['@fg-rbac', '@fleet-virt', '@cclm'] }, () => {
  ...
  test('RHACM4K-59220: Migration failure with ResourceQuota — VM remains on source', async ({ page, oc }) => {
    ...
    await test.step('1: Apply ResourceQuota on spoke to block migration', async () => {
      await applySpokeResourceQuota(VM_NAMESPACE, spokeCluster);

      const exists = await oc.resourceQuotaExists(
        'mtv-migration-deny', VM_NAMESPACE, { context: spokeCluster },
      );
      expect(exists).toBeTruthy();
    });

    await test.step('2: Navigate to VM and verify it is Running', async () => {
    ...
    await test.step('3: Trigger cross-cluster migration', async () => {
      ...
      await expect(
        cclmWizard.getReadyHeading().or(cclmWizard.getMigrateButton())
    ...
    await test.step('4: Verify error display — migration stalls on spoke', async () => {
      ...
      if (await diagnosticsTab.isVisible({ timeout: 5000 }).catch(() => false)) {
        ...
        const diagnosticsContent = page.getByText(
