...
export class ClusterDetailsPage extends BasePage {
  ...
  async gotoRoleAssignments(namespace: string, clusterName: string): Promise<void> {
    const consoleUrl = await this.oc.getConsoleUrl();
    await this.page.goto(`${consoleUrl}${RBAC_ROUTES.clusterRoleAssignments(namespace, clusterName)}`);
