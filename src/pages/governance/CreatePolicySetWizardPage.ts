...
import {
  POLICY_SET_CREATE_WIZARD,
  POLICY_SET_PLACEMENT_PREVIEW,
} from '@constants/governance';
import type { PolicySetsListPage } from '@pages/governance/PolicySetsListPage';
 ...
 */
export class CreatePolicySetWizardPage
  extends BasePage
  implements PlacementTolerationsWizardHost, PlacementPreviewWizardHost
{
  ...
  constructor(
    ...
    this.syncEditor = new SyncEditorYamlActions(page, '__policySetWizardYamlCopy');
  }

  getPageTitle(): Locator {
    return this.page.getByRole('heading', { name: POLICY_SET_CREATE_WIZARD.pageTitle, level: 1 });
  }
  ...
  getWizardStepsNav(): Locator {
    return this.page.getByLabel(POLICY_SET_PLACEMENT_PREVIEW.stepsNavAriaLabel);
  }

  getDetailsNameInput(): Locator {
    return this.getWizardContent().getByRole('textbox', {
      name: POLICY_SET_CREATE_WIZARD.details.nameTextboxLabel,
    });
  }

  getDetailsNamespaceCombobox(): Locator {
    return this.getWizardContent().getByRole('combobox', {
      name: POLICY_SET_CREATE_WIZARD.details.namespaceComboboxLabel,
    });
  }

  getNextButton(): Locator {
    return this.page.getByRole('button', { name: POLICY_SET_CREATE_WIZARD.nextButtonLabel, exact: true });
  }

  getNewPlacementButton(): Locator {
    return this.page.getByRole('button', { name: POLICY_SET_CREATE_WIZARD.placement.newPlacementLabel });
  }

  getExistingPlacementButton(): Locator {
    return this.page.getByRole('button', {
      name: POLICY_SET_CREATE_WIZARD.placement.existingPlacementLabel,
    });
  }

  getClusterSetsCombobox(): Locator {
    return this.getWizardContent().getByRole('combobox', {
      name: POLICY_SET_PLACEMENT_PREVIEW.placement.clusterSetsComboboxLabel,
    });
  }

  getExistingPlacementCombobox(): Locator {
    return this.getWizardContent().getByRole('combobox', {
      name: POLICY_SET_PLACEMENT_PREVIEW.placement.existingPlacementComboboxLabel,
    });
  }

  getSetLimitCheckbox(): Locator {
    return this.getWizardContent().getByLabel(
      POLICY_SET_PLACEMENT_PREVIEW.placement.setLimitCheckboxLabel
    );
  }
  ...
  getPlacementMatchSummary(): Locator {
    return this.getWizardFooter().filter({
      hasText: POLICY_SET_PLACEMENT_PREVIEW.footer.matchedByPlacementLabel,
    });
  }

  getPlacementPreviewLink(): Locator {
    return this.getPlacementMatchSummary().getByRole('button', {
      name: POLICY_SET_PLACEMENT_PREVIEW.footer.previewLinkPattern,
    });
  }
  ...
  async openFromPolicySetsList(listPage: PolicySetsListPage): Promise<void> {
    await listPage.gotoGovernanceOverview();
    await listPage.openPolicySetsTab();
    await listPage.clickCreatePolicySet();
  ...
  async clickWizardStep(step: 'placement' | 'review'): Promise<void> {
    const stepId =
      step === 'placement'
        ? POLICY_SET_PLACEMENT_PREVIEW.placementStepNavId
        : POLICY_SET_PLACEMENT_PREVIEW.reviewStepNavId;
  ...
  /** Policy set wizard: Placement → Review (no intermediate step). */
  async advanceToReviewStep(): Promise<void> {
    await this.getNextButton().click();
    await this.waitForLoad();
    const reviewNav = this.getWizardStepsNav().getByRole('button', { name: 'Review', exact: true });
    if (await reviewNav.isVisible().catch(() => false)) {
      await expect(reviewNav).toHaveAttribute('aria-current', 'step', { timeout: 60_000 });
    }
  }
}
